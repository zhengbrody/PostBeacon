...
| Deletion that doesn't actually delete | Partial cleanup | Explicit FK-ordered wipe (outcomes→tasks→experiments→campaigns→projects→entitlements) **plus** `auth.admin.deleteUser`; covered by tests asserting table coverage matches schema.sql | Supabase/provider backups age out on their schedules — disclosed as "residual copies in encrypted backups for up to N days" (counsel to confirm wording) |
