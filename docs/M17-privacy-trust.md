...
| Cross-user content/outcome benchmarking or training on user content | Product temptation | **Not done, and stated as a commitment**: no cross-user training and no aggregation of plan text, draft content, qualitative feedback, or outcome values by default. M24 counts only content-free lifecycle stages and returns anonymous operator totals. Any future anonymized-outcomes moat requires a separate, explicit, revocable opt-in + de-identification + minimum-cohort threshold (k ≥ 20) before outcome data is computed | Outcome-learning feature remains blocked until an explicit opt-in design is implemented and reviewed |
